...
public final class HubComponent extends ResizableHudComponent {
    ...
    public HubComponent() {
        ...
        this.setW(100);
        this.setH(200);
        this.setX((Minecraft.getMinecraft().displayWidth / 2) - (this.getW() / 2));        this.setY((Minecraft.getMinecraft().displayHeight / 2) - (this.getH() / 2));
    }
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        final boolean insideTitlebar = mouseY <= this.getY() + BORDER + Seppuku.INSTANCE.getFontManager().getHeight() + 1;
        if (!insideTitlebar) {
        ...
        Seppuku.INSTANCE.getFontManager().drawString(this.getName(), this.getX() + 2, this.getY() + 2, 0xFFFFFFFF, true, false);
        offsetY += Seppuku.INSTANCE.getFontManager().getHeight() + 1;
        ...
        for (HudComponent component : Seppuku.INSTANCE.getHudManager().getComponentList()) {
            if (component != this) {
                RenderUtil.drawRect(this.getX() + BORDER + TEXT_GAP, this.getY() + offsetY + BORDER + TEXT_GAP - this.scroll, this.getX() + BORDER + TEXT_GAP + this.getW() - BORDER - SCROLL_WIDTH - BORDER - 2, this.getY() + offsetY + BORDER + TEXT_GAP + Seppuku.INSTANCE.getFontManager().getHeight() - this.scroll, component.isVisible() ? 0x45002e00 : 0x452e0000);
                Seppuku.INSTANCE.getFontManager().drawString(component.getName(), this.getX() + BORDER + TEXT_GAP + 1, this.getY() + offsetY + BORDER + TEXT_GAP - this.scroll, component.isVisible() ? 0xFF55FF55 : 0xFFFF5555, true, false);
                offsetY += Seppuku.INSTANCE.getFontManager().getHeight() + TEXT_GAP;
            }
        }
    ...
    public void mouseRelease(int mouseX, int mouseY, int button) {
        ...
        if (inside && button == 0) {
            ...
            for (HudComponent component : Seppuku.INSTANCE.getHudManager().getComponentList()) {
                if (component != this) {
                    final boolean insideTitlebar = mouseY <= this.getY() + BORDER + Seppuku.INSTANCE.getFontManager().getHeight() + 1;
                    final boolean insideComponent = mouseX >= (this.getX() + BORDER) && mouseX <= (this.getX() + this.getW() - BORDER - SCROLL_WIDTH) && mouseY >= (this.getY() + BORDER + Seppuku.INSTANCE.getFontManager().getHeight() + 1 + offsetY - this.scroll) && mouseY <= (this.getY() + BORDER + (Seppuku.INSTANCE.getFontManager().getHeight() * 2) + 1 + offsetY - this.scroll);
