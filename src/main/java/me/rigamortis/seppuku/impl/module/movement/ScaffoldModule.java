...
public final class ScaffoldModule extends Module {
    ...
    public final Value<Mode> mode = new Value<>("Mode", new String[]{"mode"}, "Mode", Mode.ONE);
    private static int[] blackList = new int[]{145, 130, 12, 252, 54, 146, 122, 13, 219, 220, 221, 222, 223, 224, 225, 226, 227, 228, 229, 230, 231, 232, 233, 234, 50};
            ...
            if ((mc.player.movementInput.moveForward != 0 || mc.player.movementInput.moveStrafe != 0)) {
                ...
                if (mc.player.getHeldItemMainhand().getItem() != Items.AIR && mc.player.getHeldItemMainhand().getItem() instanceof ItemBlock && canPlace(mc.player.getHeldItemMainhand())) {
                    ...
                    if (block != null) {
                        ...
                        BlockUtil.placeBlockWithScaffold(pos);

                        if (mode.getValue() == Mode.THREE) {
                            BlockUtil.placeBlockWithScaffold(pos.add(-1, 0, 0));
                            BlockUtil.placeBlockWithScaffold(pos.add(1, 0, 0));
                            BlockUtil.placeBlockWithScaffold(pos.add(0, 0, -1));
                            BlockUtil.placeBlockWithScaffold(pos.add(0, 0, 1));
                            BlockUtil.placeBlockWithScaffold(pos.add(-1, 0,  -1));
                            BlockUtil.placeBlockWithScaffold(pos.add(-1, 0, 1));
                            BlockUtil.placeBlockWithScaffold(pos.add(1, 0, 1));
                            BlockUtil.placeBlockWithScaffold(pos.add(1, 0, -1));
                        }
    ...
    public static int findStackInventory() {
        for (int i = 9; i < 36; i++) {
            final ItemStack stack = Minecraft.getMinecraft().player.inventory.getStackInSlot(i);
            if (canPlace(stack) && stack.getItem() instanceof ItemBlock) {
                return i;
    ...
    public static int findStackHotbar() {
        for (int i = 0; i < 9; i++) {
            final ItemStack stack = Minecraft.getMinecraft().player.inventory.getStackInSlot(i);
            if (canPlace(stack) && stack.getItem() instanceof ItemBlock) {
                return i;
    ...
    private static boolean canPlace(ItemStack stack) {
        for (int id = 0; id < blackList.length; id++) {
            if (Item.getIdFromItem(stack.getItem()) == blackList[id]) {
                return false;
            }
        }
        return true;
