...
public final class FlightModule extends Module {
    ...
    private Value<Integer> sendEvery =new Value<>("SendEvery", new String[]{"s"}, "Speed multiplier for flight, higher values equals more speed.", 5, 1, 100, 1);

    private enum Mode {
        VANILLA, PACKET, PACKETTEST
    }
    ...
    public final Value<Boolean> tpSpeed = new Value<Boolean>("TPSpeed", new String[]{"tp"}, "Bypass the server kicking you for flying while in flight.", true);
    ...
    public void onEnable() {
        super.onEnable();
        if (this.mode.getValue() == Mode.PACKET || this.mode.getValue() == Mode.PACKETTEST) {
    ...
    public void onWalkingUpdate(EventUpdateWalkingPlayer event) {
        if (event.getStage() == EventStageable.EventStage.PRE) {
                            ...
                            if (mc.player.movementInput.jump && (mc.player.moveStrafing != 0 || mc.player.moveForward != 0)) {
                                ...
                                if (mc.player.movementInput.jump) {
                                    ...
                                    if (!tpSpeed.getValue()) {
                                        ...
                                        for (double i = 0.0625; i < this.speed.getValue(); i += this.speed.getValue()/sendEvery.getValue()) {
                                            final double[] dir = MathUtil.directionSpeed(i);
                                            move(dir[0], mc.player.posY, dir[1]);
                                        }
                                    }
                                }
                            }
            ...
            if (this.mode.getValue() == Mode.PACKETTEST) {
                if (this.teleportId <= 0) {
                    final CPacketPlayer bounds = new CPacketPlayer.Position(mc.player.posX, 0, mc.player.posZ, mc.player.onGround);
                    this.packets.add(bounds);
                    mc.player.connection.sendPacket(bounds);
                ...
                mc.player.setVelocity(0,0,0);
                double verysmol = -.0000000015;
                if (mc.player.ticksExisted % 20 == 0 && this.noKick.getValue()){
                    mc.player.setVelocity(0.0D, -0.04d, 0.0d);
                    this.move(0.0D, -0.04d, 0.0d);
                } else if (!mc.gameSettings.keyBindJump.isKeyDown() && !mc.gameSettings.keyBindSneak.isKeyDown()) {
                    if (mc.player.moveForward != 0.0f && mc.player.moveStrafing != 0.0f) {
                        for(double x = 0.0625D; x < this.speed.getValue(); x += 0.262D) {
                            double[] dir = MathUtil.directionSpeed(x);
                            mc.player.setVelocity(dir[0], verysmol, dir[1]);
                            this.move(dir[0], verysmol, dir[1]);
                        }
                    }
                } else if (mc.gameSettings.keyBindJump.isKeyDown()) {
                    for(int i = 0; i <= 3; ++i) {
                        mc.player.setVelocity(0.0D, 0.062F * i, 0.0D);
                        this.move(0.0D, 0.062F * i, 0.0D);
                    }
                } else if (mc.gameSettings.keyBindSneak.isKeyDown()) {
                    for(int i = 0; i <= 3; ++i) {
                        mc.player.setVelocity(0.0D, verysmol - 0.0625D * i, 0.0D);
                        this.move(0.0D, verysmol - 0.0625D * i, 0.0D);
                    }
                }
