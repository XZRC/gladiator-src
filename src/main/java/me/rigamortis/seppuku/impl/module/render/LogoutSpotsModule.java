...
public final class LogoutSpotsModule extends Module {
    ...
    public void onRenderWorld(EventRender3D event) {
        ...
        for (String uuid : this.logoutCache.keySet()) {
            ...
            GlStateManager.color(255, 1, 1, 1);
    ...
    public void onRender2D(EventRender2D event) {
        ...
        for (String uuid : this.logoutCache.keySet()) {
            ...
            if (projection != null && projection.isType(GLUProjection.Projection.Type.INSIDE)) {
                ...
                String text = data.profile.getName() + "\u00A7c Logout" + ChatFormatting.DARK_GRAY + "\u00A78 X " + ChatFormatting.RESET + df.format(data.position.x) + ChatFormatting.RESET + "," +
                        ChatFormatting.DARK_GRAY + "\u00A78 Y " + ChatFormatting.RESET + df.format(data.position.y) + ChatFormatting.RESET + "\u00A78," +
                        ChatFormatting.DARK_GRAY + "\u00A78 Z  " + ChatFormatting.RESET + df.format(data.position.z) + ChatFormatting.RESET + ChatFormatting.BLUE + new SimpleDateFormat(" h:mm a ").format(new Date()) + ChatFormatting.RESET;


                Seppuku.INSTANCE.getFontManager().getHeight();
                float textWidth = Seppuku.INSTANCE.getFontManager().getStringWidth(text);
                Seppuku.INSTANCE.getFontManager().drawString(text, -(textWidth / 2), 0, -1, true, false);
                GlStateManager.translate(-projection.getX(), -projection.getY(), 0);
                GlStateManager.popMatrix();
            }
        }
