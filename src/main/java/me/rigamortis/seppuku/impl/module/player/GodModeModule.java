...
public final class GodModeModule extends Module {
    ...
    public void sendPacket(EventSendPacket event) {
        if (event.getStage() == EventStageable.EventStage.PRE) {
            if (this.mode.getValue() == Mode.PORTAL) {
                if (event.getPacket() instanceof CPacketConfirmTeleport || event.getPacket() instanceof CPacketPlayer.Rotation) {
    ...
    public void receivePacket(EventReceivePacket event) {
        if (event.getStage() == EventStageable.EventStage.PRE) {
            if (this.mode.getValue() == Mode.PORTAL && Minecraft.getMinecraft().player.getRidingEntity() instanceof EntityBoat) {
                if (event.getPacket() instanceof SPacketEntityHeadLook) {
                    event.setCanceled(true);
                }
