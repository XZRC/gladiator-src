...
public final class ChatTimeStampsModule extends Module {
    ...
    public ChatTimeStampsModule() {
        super("ChatTimeStamps", new String[]{"ChatStamp", "ChatStamps"}, "Appends a time stamp on chat messages", "NONE", -1, ModuleType.CHAT);
    }
            ...
            if (event.getPacket() instanceof SPacketChat) {
                ...
                if (packet.getChatComponent() instanceof TextComponentString) {
                    ...
                    switch (this.mode.getValue()) {
                        case TWELVE:
                            date = new SimpleDateFormat("\u00A78h:mm a").format(new Date());
                            break;
                        case TWENTY_FOUR:
                            date = new SimpleDateFormat("\u00A78k:mm").format(new Date());
                            break;
                    }

                    component.text = "\u00A78[" + date + "\u00A78] " + component.getText();
                }
            }
        }
    }
